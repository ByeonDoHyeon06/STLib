...
class STLibDashboardServiceTest {
    @Test
    fun `bootstrap without persisted load should not initialize stats collection`() {
        val collectionInitCount = AtomicInteger(0)
        val store =
            STLibStatsStore(
                collectionProvider = {
                    collectionInitCount.incrementAndGet()
                    InMemoryCollectionStorage("stlib_dashboard_stats")
                },
                logWarning = {},
            )
        ...
        assertEquals(0, collectionInitCount.get())
    }
